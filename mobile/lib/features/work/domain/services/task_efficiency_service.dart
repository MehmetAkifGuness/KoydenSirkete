...
class TaskEfficiencyService {
  EffectiveWorkTask calculate(PlayerState state, WorkTask task) {
    ...
    final wheelDurationReduction = state.wheelDurationBuffTasks > 0
        ? state.wheelDurationBuffPercent / 100
        : 0;
    final wheelEnergyReduction = state.wheelEnergyBuffTasks > 0
        ? state.wheelEnergyBuffPercent / 100
        : 0;
    final durationReduction =
        (1 - (1 - skillDurationReduction) * (1 - wheelDurationReduction)).clamp(
          0,
          .75,
        );
    final energyReduction =
        (1 - (1 - skillEnergyReduction) * (1 - wheelEnergyReduction)).clamp(
          0,
          .75,
        );
    return EffectiveWorkTask(
      durationHours: (task.durationHours * (1 - durationReduction))
          .ceil()
          .clamp(1, task.durationHours),
      energyCost: (task.energyCost * (1 - energyReduction)).ceil().clamp(
        5,
        task.energyCost,
      ),
    );
