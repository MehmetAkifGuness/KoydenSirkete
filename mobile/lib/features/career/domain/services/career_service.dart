...
class CareerService {
  PromotionCheck check(PlayerState state, Job currentJob, Job? nextJob) {
    ...
    if (nextJob == null) {
      return const PromotionCheck(isEligible: false, reason: 'Bu kariyer hattının son seviyesindesin.');
    }
    if (state.performance < 70) {
      return const PromotionCheck(isEligible: false, reason: 'Terfi için performansın en az %70 olmalı.');
    }
