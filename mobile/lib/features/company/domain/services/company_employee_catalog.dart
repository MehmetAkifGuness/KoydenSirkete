...
abstract final class CompanyEmployeeCatalog {
  ...
  static const candidates = <CompanyEmployee>[
    ...
    CompanyEmployee(
      id: 16,
      name: 'Buse Kılıç',
      role: 'Müşteri deneyimi yöneticisi',
      performance: 81,
      dailySalary: 48,
      requiredCompanyLevel: 2,
    ),
  ...
  static List<CompanyEmployee> available(
    int companyLevel,
    Iterable<int> hiredIds,
  ) {
    ...
    return [...candidates, ...rotating]
        .where(
          (candidate) =>
              candidate.requiredCompanyLevel <= companyLevel &&
              !hired.contains(candidate.id),
        )
  ...
  static List<CompanyEmployee> availableForCity({
    ...
    return _generatedCandidates(
        ...
        .where((candidate) => !hired.contains(candidate.id))
        .toList(growable: false);
  }
  ...
  static List<CompanyEmployee> _generatedCandidates({
    ...
    return List<CompanyEmployee>.generate(count, (index) {
      ...
      final id = cityId == 0 ? 900000 + index : 100000 + cityId * 1000 + index;
